        ...
        <div class="container">
            <form>
                <div class="form-group">
                    ...
                    <label for="post-tag">{{ ("Topic") }}</label>
                    <select
                        class="form-control"
                        id="post-tag"
                        v-model="tag"
...
export default {
    ...
    methods: {
        getPostById() {
            axios.get('/api/get-post-by-id/' + this.$route.params.postId).then(response => {
                if (response.data) {
                    this.tag = response.data[0].tags;
        ...
        submit() {
            ...
            axios.post("/api/update-post", save_data).then(response => {
